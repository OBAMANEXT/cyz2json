...
+ =--raw= Dd not apply the moving weighted average filtering algorithm to pulse shapes, export raw, unsmoothed data,
+ =-V= display version information,
+ =--imaging-set-information= export set information for imaging,
+ =--imaging-set-definition= only if --imaging-set-information is true for file with set definitions, overrides the definitions stored in the file.
...
To update you can use the [[https://github.com/LucindaLanoy/cyz2json/blob/main/.github/workflows/api_update_from_tags.yml/][api_update_from_tags.yml]] workflow which fetches the latest version of the API (in tags), you can run it manually or let it work on schedule (15th of each month), in that case, it compares the latest tag with the current version in this repo to choose whether to update or not.
...
+ copy the files from the dll folder to the CyzFile-v*-net8.0 folder, with v* the current version of the API
+ rebuild the project.
